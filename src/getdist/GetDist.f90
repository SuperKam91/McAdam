...
!uncomment to switch order for compilation so confid_val is faster - suggested by Reijo Keskitalo March07
        ...
        integer, parameter :: max_rows = 1000000
        ...
        real mean(max_cols), sddev(max_cols), MAPest(max_cols)
        ...
        logical ::   prob_label = .false., plot_mean = .true.
         ...
         do i = 1, ncols-2
          ...
          !find upper and lower bounds
             ...
             do 
               ...
               if ((try == lasttry).and.(try/samps.lt.1)) exit
               lasttry = try
             end do
             else
               do 
...
!              write (aunit,fmt) (/(I, I=ix_min(j2), ix_max(j2), 1)/)*width(j2)+center(j2)
 ...
 !             write (aunit,fmt) (/(I, I=ix_min(j), ix_max(j), 1)/)*width(j)+center(j)
	      ...
	      if(plot_mean) write (aunit,'(a,E20.12,a,E20.12,a)') "plot(", mean(colix(j2)-2), ",", mean(colix(j)-2), ",'gx','markersize',5,'markerfacecolor','g');"
              
              if (has_markers(colix(j2)) .and. has_markers(colix(j))) then
                 write (aunit,'(a,E20.12,a,E20.12,a)') "plot(", markers(colix(j2)),",",markers(colix(j)),",'gx','markersize',5,'markerfacecolor','r','color','r');"
                 endif           
              end if
	...
	if(plot_mean) write(aunit,'(a,E20.12,a,E20.12,a)')"line([",mean(colix(j)-2), " ", mean(colix(j)-2), "], [0 1],'Color','g');"
...
!       write (aunit,*) 'set(gca,''FontSize'',axes_fontsize); hold on;'
        ...
        if (has_markers(colix(j))) then
           write (aunit,'(a,E20.12,a,E20.12,a)')"line([", markers(colix(j)), " ", markers(colix(j)),"], [0 1],'Color','r');"
        end if
...
program GetDist
	...
	integer current_row, current_col
        ...
        plot_mean = Ini_Read_Logical('plot_mean',.true.)
          ...
          do ix = 3,ncols
           ...
           if (labels(ix) /= '' .and. isused(ix)) then
                if (plotparams_num == 0 .or. any(plotparams(1:plotparams_num)==ix-2)) then
                 num_vars = num_vars + 1
                 colix(num_vars) = ix
                 mean(ix-2) = sum(coldata(1,0:nrows-1)*coldata(ix,0:nrows-1))/numsamp
                 sddev(ix-2)  = sqrt(sum(coldata(1,0:nrows-1)*(coldata(ix,0:nrows-1) &
                           -mean(ix-2))**2)/numsamp)
                end if
             end if
          end do
...
!Do 1D bins
          do j = 1,num_vars
             if (.not.(labels(j+2) /= '' .and. isused(j+2))) continue
             ix = colix(j)
             bincounts = 0
             binlikes = 0
             binmaxlikes = 0
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!! JH: end of MCI stuff !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	     ...
	     if(bincounts(i)==maxbin) MAPest(j)=center(j) + i*width(j)
          end do
          ...
          plot_y = plot_row - (j+plot_col-1)/plot_col + 1 
           write (51,*) 'spo=subplot(',plot_row,',',plot_col,',',j,')'  
           call Write1DplotMatLab(51,j);
           if (matlab_version < 7) then
              if (plot_row*plot_col > 4) then
!	      	   	      current_row = nint(((j-1)/(1.0*plot_col)) + 1.0)
! this one doesn't seem to get called
!	      	if (current_row == plot_row) then
			call CheckMatlabAxes(51)
!		else
!			write (51,*) 'set(gca,''xtick'',[]);hold off;'
!			write (51,*) 'set(gca,''ytick'',[]);hold off;'
!		end if
	      end if
	   ! make the plots bigger
	       write(51,*) 'p = get(spo, ''pos'');'
	       write(51,*) 'p(3) = p(3) + 0.01;'
	       write(51,*) 'p(4) = p(4) + 0.01;'
	       write(51,*) 'set(spo, ''pos'', p);'
	     write (51,*) ' p = get(gca,''Position'');'
           ...
           if (triangle_plot) then
            write (52,*) 'spo=subplot(',num_vars,',',num_vars,',',(j-1)*num_vars+j,')'
            call Write1DplotMatLab(52,j);
            if (num_vars > 2) then
    	      	if (j == num_vars) then
			call CheckMatlabAxes(52)
			write (52,*) 'set(gca,''ytick'',[]);hold off;'
		else
			write (52,*) 'set(gca,''xtick'',[]);hold off;'
			write (52,*) 'set(gca,''ytick'',[]);hold off;'
		end if
	      end if
	   ...
	   ! make the plots bigger
	       write(52,*) 'p = get(spo, ''pos'');'
	       write(52,*) 'p(3) = p(3) + 0.01;'
	       write(52,*) 'p(4) = p(4) + 0.01;'
	       write(52,*) 'set(spo, ''pos'', p);' 	    
         ...
         if (num_2D_plots > 0 .and. .not. no_plots) then
          ...
          plot_num = 0
           ...
           do j2 = j2min, num_vars
              ...
              write (50,'(''spo=subplot('',1I5,'','',1I5,'','',1I5,'');'')') plot_row,plot_col,plot_num
              call Write2DPlotMATLAB(50,j,j2,.true.,.true.)
	      current_row = nint(((plot_num-1)/(1.0*plot_col)) + 1.0)

              if (plot_row*plot_col > 4) then
	!      	if (current_row == plot_row) then
			call CheckMatlabAxes(50)
	!	else
	!		write (50,*) 'set(gca,''xtick'',[]);hold off;'
	!		write (50,*) 'set(gca,''ytick'',[]);hold off;'
	!	end if
	      end if
	       ! make the plots bigger  
	       write(50,*) 'p = get(spo, ''pos'');'
	       write(50,*) 'p(3) = p(3) + 0.01;'
	       write(50,*) 'p(4) = p(4) + 0.01;'
	       write(50,*) 'set(spo, ''pos'', p);' 
              end if
           end do
           end if
          end do
         ...
         if (triangle_plot .and. .not. no_plots) then
          ...
          do j = 1, num_vars
            do j2 = j+1, num_vars
              ...
              write (52,'(''spo=subplot('',1I5,'','',1I5,'','',1I5,'');'')') num_vars, num_vars, &
                   (j2-1)*num_vars + j 
              call Write2DPlotMATLAB(52,j2,j,j2==num_vars, j==1)
	      current_row = int(((((j2-1)*num_vars + j)-1)/(1.0*num_vars))+1)
              current_col = mod(((j2-1)*num_vars + j-1),num_vars)
	      !write(*,*) 'plotnumber', (j2-1)*num_vars + j
	      !write(*,*) 'current_row', current_row
	      !write(*,*) 'current_col', current_col
              if (num_vars > 2) then
	      	if (current_row == num_vars) then
			call CheckMatlabAxes(52)
		else
			write (52,*) 'set(gca,''xtick'',[]);hold off;'
		end if
...
!Do 3D plots (i.e. 2D scatter plots with coloured points)
          ...
          if (num_3D_plots /=0 .and. .not. no_plots) then
              ...
              do j=1, num_3D_plots
                 ...
                 write (50,'(''spo=subplot('',1I5,'','',1I5,'','',1I5,'');'')') plot_row,plot_col,j  
...
!Marginalized
         ...
         open(unit=50,file=trim(rootdirname)//'.margestats',form='formatted',status='replace')
          write(50,'(a)',advance='NO') 'param  mean          sddev         MAP           '    
          do j=1, num_contours
          ...
          do j=1, num_vars
             write(50,'(1I5,3E14.6)', advance='NO') colix(j)-2, mean(colix(j)-2), sddev(colix(j)-2), MAPest(j)
             do i=1, num_contours
