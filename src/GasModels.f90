...
!           (vr+i*vi)*(cos+i*sin)=(vr*cos-vi*sin)+i*(vr*sin+vi*cos)
            ...
            pvisr(m,k)=pvisr(m,k) + dble(rr)*dble(cos(theta))-dble(ii)*dble(sin(theta))
            pvisi(m,k)=pvisi(m,k) + dble(rr)*dble(sin(theta))+dble(ii)*dble(cos(theta))
...
!=======================================================================
	...
	subroutine AddSource(i)

! Add a single point source to the predicted visibilities.
...
! Loop over visibilities, calculating amplitude and phase
