...
subroutine Rescale_nest(value,ival1,cont)
    ...
    ! 	First determine whether 1D coordinate ival refers to an atomic or 
...
subroutine PClass(ival,Ptype)
	...
	tot_pars=NPars
      	...
      	if(ival>tot_atoms*tot_pars) then
        	nuis=.true.
      	else
        	nuis=.false.
      	endif
	...
	if( .not. nuis ) then
	   	...
	   	i = ival - ( j - 1 ) * tot_pars
			...
			Ptype = Geo_PriorType(j,ii)
         	...
         	elseif( SZ == 1 .and. Gas == 1 .and. i <= ( NGeoPars + 1 + NMassPars * Mass + NGasPars * Gas ) ) then
			ii = i - NGeoPars - 1 - NMassPars * Mass	 
			Ptype = Gas_PriorType(j,ii)
      		elseif( SZ == 1 .and. Gas == 1 .and. Temperature ==1 .and.i <= ( NGeoPars + 1 + NMassPars * Mass + ( NGasPars + NTPars ) * Gas ) ) then
			ii = i - NGeoPars - 1 - NMassPars * Mass - NGasPars * Gas	 
...
! Loop over all atoms:
	...
	if( ( MassModel == 1 ) .or. ( SZ == 1 .and. Gas == 1 ) ) then
	    Mmin = 1.d90
	    Mmax = 0d0
	    do k = 1, NAtoms
