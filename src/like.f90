...
subroutine FUserbuild(Cool,Nsystem,Lhood,Nstore,idummy,ndummy,Cube1,retcode)
    ...
    double precision SZLhood,urv,zsrc(nzsplanes)
...
!-----------------------------------------------------------------------
    ...
    !noCluster = any(GeoPars/=GeoPars_old) .and. any(MassPars/=MassPars_old) .and. any(GasPars/=GasPars_old) .and. any(TPars/=TPars_old)  .and. any(z/=z_old) 
        ...
        if (noCluster) then
          ! start with saved fixed base + old cluster visibilities
          pvisr = pvisr1
          pvisi = pvisi1
        else
          ! start from fixed base visibilities
          pvisr = pvisr_base
          pvisi = pvisi_base
        endif

        ! Now if cluster parameters have changed, make the cluster visibilities and add them to the fixed base visibilities
	...
	if(Nuisance==1.and.SourceSubtract==1) then ! call AddSources
          do isrc=1,NSrc
            if (sum(Src_PriorType(isrc,:)).ne.0) then
              call AddSource(isrc)
            endif
          enddo
        endif
...
!=======================================================================
	...
	integer i,j,k,m,index,iend,Wastage,ival
...
!           endif
            ...
            ! Set up the completely fixed sources
            if (SourceSubtract==1) then
              do m = 1, NSrc
                if (sum(Src_PriorType(m,:)).eq.0) then
                  i=NAtoms*Npars+(m-1)*4+1
                  do ival=i, i+4
                    call Rescale(0d0, ival)
                  enddo
                  call AddSource(m)
                endif
              enddo
...
! Initialise working arrays:
	  ...
	  !Gammafun((b_GNFW-3.d0)/a_GNFW)/Gammafun((-c_GNFW + b_GNFW)/a_GNFW)
        ...
        if(.not.simulate) then
            if(is_root) write(*,*) '         Lhood0=', SZLhood0+GLLhood0	 
	  	GLLhood=0.d0
	  	SZLhood=0.d0
	  	...
	  	GLNullEv=GLLhood
        if(is_root) write(*,*) '  Null Evidence=',NullEv
        endif
