...
subroutine poly_settings(settings)
    ...
    settings%write_prior   = .true.               ! write a prior file?
    ...
    settings%update_files  = settings%nlive       ! how often to update the files
    ...
    allocate(settings%grade_frac(2),settings%grade_dims(2)) 
    settings%grade_frac=[0.5d0, 0.5d0]
    settings%grade_dims=[eslow, edim-eslow]
