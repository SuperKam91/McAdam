...
module generate_module
    ...
    subroutine GenerateLivePoints(loglikelihood,settings,RTI,mpi_information)
        use settings_module,  only: program_settings
        ...
        if(linear_mode(mpi_information)) then
            ...
            do while(RTI%nlive(1)<nprior)
                ...
                time0 = time()
                call calculate_point( loglikelihood, live_point, settings, nlike)
                time1 = time()
            ...
            if(is_root(mpi_information)) then
                ...
                do while(.true.)
                    ...
                    time0 = time()
                    call calculate_point( loglikelihood, live_point, settings,nlike) ! Compute physical coordinates, likelihoods and derived parameters
                    time1 = time()
        ...
        call time_speeds(loglikelihood,settings,RTI,speed,mpi_information) 
    ...
    subroutine time_speeds(loglikelihood,settings,RTI,speed,mpi_information)
        use settings_module,  only: program_settings
        use run_time_module,   only: run_time_info
        use random_module,   only: random_reals
    ...
    subroutine GenerateLivePointsFromSeed(loglikelihood,settings,RTI,mpi_information)
        use settings_module,  only: program_settings
        ...
        if(linear_mode(mpi_information)) then
            ...
            do while(RTI%nlive(1)<nprior)
                ...
                do i_repeat = 1,settings%nprior_repeat
                    ...
                    time0 = time()
                    live_point = slice_sample(loglikelihood,logzero,nhat,live_point,1d0,settings,nlikes(i_grade))
                    time1 = time()
