module chordal_module
    ...
    function SliceSampling(loglikelihood,settings,logL,seed_point,cholesky,nlike,num_repeats)  result(baby_points)
        ...
        do i_babies=1,size(nhats,2)
            ...
            baby_points(:,i_babies) = slice_sample(loglikelihood,logL, nhat, previous_point, w, settings,nlike(speeds(i_babies)))
    ...
    !!
    function slice_sample(loglikelihood,logL,nhat,x0,w,S,n) result(baby_point)
        use settings_module, only: program_settings
        ...
        call calculate_point(loglikelihood,R,S,n)
        call calculate_point(loglikelihood,L,S,n)
        ...
        recursive function find_positive_within(L,R) result(x1)
            ...
            call calculate_point(loglikelihood,x1,S,n)
