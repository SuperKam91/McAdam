...
! SZ setup
        ...
        if (SZ .ne. 0 ) then
          ...
          allocate(pvisr_base(Nvisfiles,large))
          allocate(pvisi_base(Nvisfiles,large))
...
! No. of parameters per object:
	NPars=NGeoPars+NMassPars*max(GL,MMass)+SZ*(NGasPars*Gas+NTPars*Temperature)+1

! No. of nuisance parameters:
	NNuisance = SZ*4*NSrc + Varyzs

! Total no. of dimensions of parameter space:
	NDim = NAtoms*NPars + NNuisance
...
!=======================================================================
    subroutine write_paramnames
      ...
      do ival = 1, Ndim
         nuis = .false.
         parunit=''
         if (ival>NAtoms*NPars) nuis=.true.
         if (.not. nuis) then
            j=(ival-1)/NPars+1
            write(js,'(I0)') j
            i=ival-(j-1)*NPars
            ...
            else if( SZ == 1 .and. Gas == 1 .and. i <= ( NGeoPars + 1 + NMassPars * Mass + NGasPars * Gas ) ) then
            ...
            else if( SZ == 1 .and. Gas == 1 .and. Temperature==1 .and. i <= ( NGeoPars + 1 + NMassPars * Mass + ( NGasPars + NTPars ) * Gas ) ) then
	   ...
	   if (GasModel==5) then
	      ...
	      write(params_unit,fmt) 'p',edim+(j-1)*aux_dim+20, '*', '\rho_{-2}'//trim(ext)
           ...
           if (which_sampler=='M') then
             write(params_unit,fmt) 'p',edim+(j-1)*aux_dim+26, '*', 'y_{\mathrm{cent,map}} / \mathrm{arcmin}^2'//trim(ext)
             write(params_unit,fmt) 'p',edim+(j-1)*aux_dim+27, '*', '\mathrm{central\, cluster\, flux} / \mathrm{Jy}'//trim(ext)
             write(params_unit,fmt) 'p',edim+(j-1)*aux_dim+28, '*', '\mathrm{central\, cluster\, temp} / \mathrm{K}'//trim(ext)
           endif
        else
           do ival = 1, aux_dim
              write(iis,'(I0)') ival
